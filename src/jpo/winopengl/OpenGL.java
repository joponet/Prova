...
//	    gl.glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, materialEmission,0);
	    ...
	    if (objecte.faces.get(0).tipus==4) {
			gl.glEnd();	    
		    gl.glBegin(GL2.GL_QUADS);
	    }
	    for (int nf=0; nf<objecte.faces.size(); nf++) {
    		gl.glColor3fv(objecte.faces.get(nf).getColor(), 0);
    		gl.glNormal3dv(objecte.faces.get(nf).getNormal(), 0);
    		gl.glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, objecte.faces.get(nf).color,0);
    		// escriu cara
    		for (int nfv=0; nfv<objecte.faces.get(nf).tipus; nfv++) {
	    		gl.glVertex3dv(objecte.faces.get(nf).getVertex(nfv),0);
	    	}
	    }
		gl.glEnd();
